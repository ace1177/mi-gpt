...
export default {
  ...
  speaker: {
    ...
    callAIKeywords: ["请", "你", "蒲蒲"],
    // 当消息以下面的关键词开头时，会进入 AI 唤醒状态
    wakeUpKeywords: ["亲爱的，我们什么时候回南充"],
    // 当消息以下面的关键词开头时，会退出 AI 唤醒状态
    exitKeywords: ["先睡觉了"],
    // 进入 AI 模式的欢迎语
    onEnterAI: ["醒了想你了"], // 设为空数组时可关闭提示语
    // 退出 AI 模式的提示语
    onExitAI: ["蒲蒲先睡觉了"], // 为空时可关闭提示语
    // AI 开始回答时的提示语
    onAIAsking: ["大概是这样子"], // 为空时可关闭提示语
    // AI 结束回答时的提示语
    onAIReplied: ["我说完了", "还有其他问题吗"], // 为空时可关闭提示语
    // AI 回答异常时的提示语
    onAIError: ["我不清楚了，过会吗！"], // 为空时可关闭提示语
